            ...
            if len(ctrl) != 0:
                for r in range(len(ctrl)):
                    ...
                    if len(self.noise_values) > 0:
                        self.ansatz.add_gate(TwoQubitDepolarizingNoise(ctrl[r], targ[r], self.noise_values[0]) )
            ...
            if len(rot_qubit_list) != 0:
                for pos, r in enumerate(rot_direction_list):
                    ...
                    if len(self.noise_values) > 1:
                            self.ansatz.add_gate(DepolarizingNoise(rot_qubit, self.noise_values[1]))
...
def get_energy_qulacs(angles, observable, 
                      ...
                      qulacs_energy_shift, n_shots,
                      phys_noise = False,
                      which_angles=[]):
    """"
    Energy minimization using Qulacs
    ...
    expval = get_exp_val(n_qubits,circuit,observable, qulacs_energy_shift)
    return expval



def get_exp_val(n_qubits,circuit,op, qulacs_energy_shift):

    state = QuantumState(n_qubits)
    circuit.update_quantum_state(state)
    expval = op.get_expectation_value(state)
    return expval+qulacs_energy_shift

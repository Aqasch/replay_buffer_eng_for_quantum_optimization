...
from environment_qulacs_compiling_haar_random import CircuitEnv
...
class ReliabilityAdjustedPrioritizedReplayBuffer:
    """
    ReaPER: Reliability-Adjusted Prioritized Experience Replay (Pleiss, Leonard S. ICLR 2026)
    """
...
class DQNAgentWithReaPER:
    ...
    def __init__(self, state_size, action_size, config, device, network_type='paper', seed=42):
        ...
        self.memory = ReliabilityAdjustedPrioritizedReplayBuffer(
                ...
                omega_start=config.get('reaper_omega_start', 0.1),
                omega_end=config.get('reaper_omega_end', 0.7),
...
def train_dqn(env, agent, replay, anneal_active, config, seed=42):
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % print_every == 0:
            ...
            avg_loss = np.mean(agent.losses[-1000:]) if agent.losses else 0# Save checkpoint
        if (episode + 1) % save_every == 0:
            agent.save(f'checkpoints/dqn_{replay}_hrc_seed_{seed}_anneal_{anneal_active}.pth')
            save_episode_metrics(episode_data, episode + 1, 'hrc', f'dqn_{replay}', anneal_active, seed)
            

            # BUFFER SAVE:
            ckpt_path = os.path.join("checkpoints", f"dqn_buffer_{replay}_1qubit_ep{episode}.pt")
            save_dict = {
                "replay_type": replay,
                "memory": agent.memory,
            }
            torch.save(save_dict, ckpt_path)
            print(f"Checkpoint (with replay buffer) saved to {ckpt_path}")
        ...
        if (episode + 1) % save_every == 0:
            ...
            if eval_results:
                save_eval_results(eval_results, replay, anneal_active, seed)
    
    # Final save
    save_episode_metrics(episode_data, num_episodes, 'hrc', f'dqn_{replay}', anneal_active, seed, final=True)
    if eval_results:
        save_eval_results(eval_results, replay, anneal_active, seed, final=True)
    
    return episode_rewards, episode_lengths, episode_fidelities, episode_data, eval_results


def save_episode_metrics(episode_data, episode_num, gateset, agent_type, anneal_active, seed, final=False):
    ...
    if final:
        pkl_path = f'results/episode_metrics_final_gateset_{gateset}_agent_{agent_type}_anneal_{anneal_active}_seed_{seed}.pkl'
        print(f"\nSaving final episode metrics to {pkl_path}")
        print(f"Total episodes recorded: {len(episode_data)}")
    else:
        pkl_path = f'results/episode_metrics_gateset_{gateset}_agent_{agent_type}_anneal_{anneal_active}_seed_{seed}.pkl'
    
    with open(pkl_path, 'wb') as f:
        pickle.dump(episode_data, f)


def save_eval_results(eval_results, replay, anneal_active, seed, final=False):
    ...
    if final:
        pkl_path = f'results/eval_results_final_seed_dqn_{replay}_anneal_{anneal_active}_{seed}.pkl'
        csv_path = f'results/eval_results_final_seed_dqn_{replay}_anneal_{anneal_active}_{seed}.csv'
    else:
        pkl_path = f'results/eval_results_seed_dqn_{replay}_anneal_{anneal_active}_{seed}.pkl'
        csv_path = f'results/eval_results_seed_dqn_{replay}_anneal_{anneal_active}_{seed}.csv'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--replay', type=str, default='her', help='Exeorience replay type: her or per')
    ...
    print(f"\n{'='*60}")
    print(f"TRAINING CONFIGURATION: DQN+{args.replay} for HRC Gates")
    print(f"{'='*60}")
    print(f"Algorithm: DQN with Hindsight Experience Replay")
    print(f"Gateset: HRC (Harrow-Recht-Chuang)")
    print(f"Reward: Sparse (Equation 4)")
    print(f"Seed: {SEED}")
    print(f"{'='*60}\n")
    ...
    env_config = {
        'env': {
            'num_qubits': 1,
            'max_episode_length': 130,  # Table 4
            'tolerance': 0.99,
    ...
    # DQN+HER config (Table 4)
        agent_config = {
            ...
            'reaper_omega_anneal': False, # placeholdere
        ...
        agent_config = {
            ...
            'epsilon_min': 0.01,
            'epsilon_decay': 0.99995,  # Table 4
            'reaper_omega_anneal': False, # placeholder
            'learning_rate': 3e-4,  # Table 4: 0.0001
            'batch_size': 200,  # Table 4
            'memory_size': 500000,  # Table 4: 5·10^5 experiences
            'target_update_freq': 100,  # Update every episode
        ...
        agent_config = {
                    ...
                    # ReaPER
                    'reaper_alpha': 0.4,
                    'reaper_omega_anneal': False,      # ← DISABLED ANNEALING
                    'reaper_omega_start': 0.6,        # Start high
                    'reaper_omega_end': 0.0,          # End low
    ...
    training_config = {
        'num_episodes': 50000,
        'max_steps_per_episode': 130,  # Table 4
        'print_every': 1,
        'save_every': 100,
        'eval_every': 500,  # NEW
    ...
    if args.replay == 'per':
        ...
        print("Creating DQN+REAPER (anneal) agent...\n")
        # Create DQN+REAPER agent
        agent = DQNAgentWithReaPER(env.state_size, env.action_size, agent_config, device, network_type='paper', seed=SEED)
        ...
        print(f"Expected 'per' or 'her' or 'reaper' or 'reaper_anneal'")
        exit()
    ...
    episode_rewards, episode_lengths, episode_fidelities, episode_data, eval_results = train_dqn(
        env, agent, args.replay, agent_config['reaper_omega_anneal'], training_config, seed=SEED
    )
    ...
    if eval_results:
        ...
        print(f"Final Mean Fidelity: {final_eval['mean_feidelity']:.6f}")
